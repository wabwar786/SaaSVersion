...
function syncTableAllowed(string $table): bool {
  static $allow=[
    ...
    /* V62.2: `user_module_access` yahan THI HI NAHI. Users page modules
       ...
       kabhi cloud tak pohanchte hi nahi the -> online "0 Modules". */
    'user_module_access','user_site_access','platform_modules',
...
function applyUser(string $id,array $d,bool $create=false,?string $requestId=null):string{$p=DB::pdo();$role=roleIdByName($d['role']??'Cashier');$mods=[];foreach($d['modules']??[] as $k)if($m=moduleId($k))$mods[]=$m;$perm=$d['permissions']??[];if($create){return UserService::create(['full_name'=>$d['name'],'email'=>$d['email'],'username'=>$d['username']??'','phone'=>$d['phone']??'','password'=>$d['password']?:'1234','role_id'=>$role,'modules'=>$mods,'is_admin'=>($d['role']??'')==='Owner / Admin','form_permissions'=>[]],$requestId);}return DB::tx(function($p)use($id,$d,$role,$mods){$p->prepare("UPDATE users SET full_name=?,email=?,phone=?,updated_at=NOW(6) WHERE id=? AND tenant_id=?")->execute([$d['name'],$d['email'],$d['phone']??'',$id,tenant_id()]);if(!empty($d['password'])){[$h,$a]=UserService::passwordHash($d['password']);/* ============================================================
          `updated_at` aur `row_version` yahan LAZMI hain.

          Sync ko sirf isi se pata chalta hai ke row badli. Yeh query
          unhein chhoti hi nahi thi — nateeja: cloud par password badal
          jata, magar branch computer par kabhi na pohanchta, chahe
          internet mahino chalta rahe. Staff naye password se online
          login kar leta aur offline POS purana hi maangta rehta, aur
          koi nahi bata sakta tha ke kyun.

          Upar wali query (naam/email/phone) `updated_at` barhati hai —
          is liye jab naam ke SAATH password badla jata to chala jata,
          aur akela password badla jata to nahi. Isi liye yeh masla
          kabhi kabhi theek lagta tha.
          ============================================================ */$p->prepare("UPDATE users SET password_hash=?,password_algo=?,row_version=row_version+1,updated_at=NOW(6) WHERE id=?")->execute([$h,$a,$id]);}$p->prepare("DELETE FROM user_roles WHERE user_id=?")->execute([$id]);$p->prepare("DELETE FROM user_module_access WHERE user_id=?")->execute([$id]);if($role)$p->prepare("INSERT INTO user_roles(id,user_id,role_id,site_id,assigned_by) VALUES(?,?,?,?,?)")->execute([uuid(),$id,$role,site_id(),current_user()['id']??null]);foreach($mods as $m)$p->prepare("INSERT INTO user_module_access(id,user_id,site_id,module_id,access_mode) VALUES(?,?,?,?, 'ALLOW')")->execute([uuid(),$id,site_id(),$m]);return$id;});}
...
case 'report-list':
 /* RETAIL tenant ki apni reports — uska data model alag hai. */
 if(Auth::tenantIndustry()==='RETAIL'){needLogin();ok(['reports'=>RetailReportService::catalog()]);}needLogin();Auth::requireModule('reports');
 ok(['reports'=>ReportService::catalog()]);

case 'report-run':
 if(Auth::tenantIndustry()==='RETAIL'){needLogin();
   $id=preg_replace('/[^a-z_]/','',strtolower($_GET['id']??''));
   try{$r=RetailReportService::run($id,(string)($_GET['from']??''),(string)($_GET['to']??''));}
   catch(Throwable $e){fail($e->getMessage());}
   ok(['report'=>$r]);}needLogin();Auth::requireModule('reports');
...
case 'report-csv':
 if(Auth::tenantIndustry()==='RETAIL'){needLogin();
   ...
   header('Content-Disposition: attachment; filename="'.$id.'_'.$r['from'].'_'.$r['to'].'.csv"');
   echo RetailReportService::csv($r); exit;}needLogin();Auth::requireModule('reports');
 $id=preg_replace('/[^a-z_]/','',strtolower((string)($_GET['id']??'')));
 try{$r=ReportService::run($id,(string)($_GET['from']??''),(string)($_GET['to']??''));}
 catch(Throwable $e){fail($e->getMessage());}
...
case 'pos-boot':needLogin();if(!Auth::canModule('pos')&&!Auth::canModule('tablet'))fail('Permission denied',403);$bu=Auth::user();$bb=PageData::posBoot();$sq=DB::pdo()->prepare("SELECT name FROM sites WHERE id=? LIMIT 1");$sq->execute([site_id()]);$bb['site']=['name'=>(string)($sq->fetchColumn()?:'Main Branch')];$sg=$p2=DB::pdo()->prepare("SELECT data_json FROM ui_records WHERE tenant_id=? AND site_id=? AND module_key='pos_settings' AND deleted=0 ORDER BY created_at DESC LIMIT 1");$sg->execute([tenant_id(),site_id()]);$sj=$sg->fetchColumn();$sd=$sj?(json_decode($sj,true)?:[]):[];$bb['settings']=['tax_cash'=>isset($sd['tax_cash'])?(float)$sd['tax_cash']:16.0,'tax_card'=>isset($sd['tax_card'])?(float)$sd['tax_card']:8.0,'service_charge'=>isset($sd['service_charge'])?(float)$sd['service_charge']:0.0,/* POS ka SET isi payload se banta hai — `settings-get` se nahi.    Yehi wajah thi ke Settings mein EXCLUSIVE chunne ka POS par koi    asar nahi hota tha: mode wahan pohanchta hi nahi tha. */'tax_mode'=>\Aio\Services\TaxMode::current()];
/* POS ko FBR ki halat bhi chahiye — warna QR khamoshi se gayab rehta hai
   aur cashier ko wajah ka pata hi nahi chalta. */
$bb['fiscal']=['enabled'=>FiscalService::availableHere(),'tenant_on'=>FiscalService::enabledForTenant(),'provider'=>(string)(FiscalService::settings()['provider']??'NONE')];$bq=DB::pdo()->prepare("SELECT name,display_name,logo_url,brand_color,brand_accent FROM tenants WHERE id=? LIMIT 1");$bq->execute([tenant_id()]);$br=$bq->fetch()?:[];
...
case 'menu-categories':needLogin();
 ...
 $q=DB::pdo()->prepare("SELECT id,name FROM menu_categories
...
case 'sync-schema':$stid=syncTenant();syncNodeSeen($stid);
 ...
 /* V62.2 — MODULE FINGERPRINT.
    `platform_modules.id` pehle har installation par random thi, is liye
